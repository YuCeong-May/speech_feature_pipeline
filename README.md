...
- 新增基于 jieba 词边界的词内/词间停顿统计：在句子级 metrics CSV 和 `*.metrics.json` 句子版本 summary 中输出 `within_word_*` 与 `between_word_*` 停顿时长、次数和无阈值 gap 参考字段，并额外保存 `*.word_segments.csv` 与 `*.word_gaps.csv` 分词/gap 明细。
...
*.independent_filler.word_segments.csv
*.independent_filler.word_gaps.csv
*.merge_filler_to_next.word_segments.csv
*.merge_filler_to_next.word_gaps.csv
```

其中 `*.summary.metrics.csv` 是后续工作流最常用的全局指标表。

`*.word_segments.csv` 保存 jieba 分词后的词明细，包括每个词的 `word_id`、`word_text`、字符范围、起止时间和覆盖的对齐 token 数；`*.word_gaps.csv` 保存相邻对齐 token 的 gap 明细，包括 `gap_scope`（`within_word` 或 `between_word`）、`gap_sec`、是否超过停顿阈值，以及前后 token/word 的文本和时间。
