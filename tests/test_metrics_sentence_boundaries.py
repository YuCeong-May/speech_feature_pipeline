...
from src.align.metrics import (
    add_inter_sentence_gaps,
    assign_items_to_sentences,
    build_sentence_lines,
    sentence_metrics,
    sentence_word_detail_rows,
...
def test_jieba_word_pause_metrics_separate_within_and_between_word(monkeypatch):
    ...
    segment_rows, gap_rows = sentence_word_detail_rows(sentence, 0.2)
    assert segment_rows == [
        {
            "sentence_id": 1,
            "word_id": 1,
    ...
    assert gap_rows[1]["gap_scope"] == "within_word"
    assert gap_rows[1]["gap_sec"] == 0.4
    assert gap_rows[1]["prev_word_text"] == "觉得"
    assert gap_rows[1]["cur_word_text"] == "觉得"
    assert gap_rows[2]["gap_scope"] == "between_word"
    assert gap_rows[2]["gap_sec"] == 0.4
    assert gap_rows[2]["prev_word_text"] == "觉得"
    assert gap_rows[2]["cur_word_text"] == "好"
