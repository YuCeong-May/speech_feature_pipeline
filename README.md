...
本项目当前不包含 ASR 自动转录步骤。`.txt` 转录文本由外部提供，`run.py --run_forced_align` 会调用内部对齐模块，把已有文本与音频对齐并生成时间戳。
...
| 参数 | 默认值 | 说明 |
|---|---|---|
| `--save_frame_level` | 关闭 | 开启帧级 CSV 输出，会为每个音频生成频谱帧表和 Praat 帧表。 |
...
| `--run_forced_align` | 关闭 | 传统声学特征完成后，再执行 Forced-Aligner 和对齐后韵律指标。 |
...
| 对齐后韵律 | 语速、停顿时长、停顿次数、发音时间、平均音节时长、停顿占比 | Qwen3-ForcedAligner + `run.py --run_forced_align` 内部指标模块 | `output/metrics/` |
...
### 3.2 Qwen3-ForcedAligner 强制对齐和对齐后韵律指标
...
如果开启 `--run_forced_align`，`run.py` 会按照下面顺序执行：

1. 先提取传统声学特征，并同步绘制频谱图。
2. 再按音频 `file_id` 到 `--transcript_dir` 中寻找同名 `.txt` 转录文本。
3. 调用 Qwen3-ForcedAligner 生成 JSON/TSV 时间戳。
4. 调用内部指标模块计算语速、停顿、发音时间、平均音节时长等对齐后韵律指标。
...
| 参数 | 默认值 | 说明 |
|---|---|---|
| `--run_forced_align` | 关闭 | 传统声学特征提取后继续运行强制对齐和对齐后韵律指标。 |
| `--transcript_dir` | `--input_dir` | 转录文本目录，按音频同名 `.txt` 匹配。 |
...
如果开启 `--run_forced_align` 且找到同名转录文本，脚本会额外输出两个句子级 CSV：
